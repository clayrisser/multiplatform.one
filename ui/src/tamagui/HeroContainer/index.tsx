...
const AnimationControl = () => {
  ...
  return (
    <TooltipSimple placement="top" label={`Animations: ${niceNames[animationDriverToggler.driverName]}`}>
      <XStack zIndex={100000000} space="$2" ai="center">
        <Timer size={14} opacity={0.6} />
        <Switch
          size="$1"
          checked={animationDriverToggler.driverName === 'react-native'}
          onCheckedChange={(val) => animationDriverToggler.setDriverName(val ? 'react-native' : 'css')}
        >
...
const HeroContainerInner = ({ children, demoMultiple }: { children: React.ReactNode; demoMultiple: boolean }) => {
