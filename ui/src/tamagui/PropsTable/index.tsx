    ...
    >
      ...
      {data.map(({ name, type, required, default: defaultValue, description }, i) => (
        <ListItem key={`${name}-${i}`} p={0}>
          <YStack width="100%">
            <XStack position="relative" py="$3" px="$4" $sm={{ flexDirection: 'column' }}>
              <YStack fullscreen backgroundColor="$background" zi={-1} o={0.5} />
              <XStack miw="30%" alignItems="center" space>
              ...
              <Separator alignSelf="stretch" vertical mx="$4" my="$2" />

              <XStack
                f={2}
                miw="30%"
                ai="center"
                separator={<Separator alignSelf="stretch" vertical mx="$4" my="$2" />}
                $xs={{
                  flexDirection: 'column',
                  alignItems: 'flex-start',
                }}
              >
                <Paragraph size="$3" o={0.8} fontFamily="$mono" overflow="hidden" ellipse marginRight="auto">
                  {type}
                </Paragraph>
                ...
                <XStack alignItems="center">
                  {Boolean(defaultValue) ? (
                    <>
                      ...
                      <Code my="$-1" backgroundColor="$backgroundPress">
                        {defaultValue}
                      </Code>
                    </>
                  ) : null}
